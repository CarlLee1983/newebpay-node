...
describe("CLI", () => {
    ...
    describe("express command", () => {
        ...
        it("當目錄不存在時應建立範例專案", () => {
            ...
            expect(mockFs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining("test-dir"), { recursive: true });
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining("package.json"), expect.any(String));
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining("index.js"), expect.any(String));
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining(".env.example"), expect.any(String));
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining("README.md"), expect.any(String));
        });
